...
\label{sec:constraints_on_kappa}
	In this section, the $m_{HH}$ distribution are used to set the constraints of $\kappa_\lambda$.

	The binned $m_{HH}$ distribution are used. The likelihood function $L$ consisting of a product of Poisson distributions 
	\begin{equation}
	...
	\label{sub:mhh_distribution}
		Figure \ref{fig:mhh_distribution_min_dR} and \ref{fig:mhh_distribution_SPANET} are the $m_{HH}$ distrubiton of $\text{min-}\Delta R$ and SPA-NET pairing method, respectively. The events passing all selection cuts are used. For signal, both pairing method give the similar results. For background, the results look different.
	...
	\label{sub:expected_limit_of_kappa}
		For expected limit setting, $n_i$ is the background data. The likelihood of different $\kappa_\lambda$ are calulated. The range of $\kappa_\lambda$ is $[-10,15]$ by step $1$.

		Figure \ref{fig:log_likelihood_ratio_kappa_min_dR} and \ref{fig:log_likelihood_ratio_kappa_SPANET} are the profile likelihood ratio scans for $\kappa_\lambda$ with $\text{min-}\Delta R$ and SPA-NET pairing method. The $2\sigma$ exclusion limit: $\kappa_\lambda \in \left[ -5.01,12.21 \right]$ for $\text{min-}\Delta R$ method; $\kappa_\lambda \in \left[ -5.30, 13.02 \right]$ for SPA-NET method.
		\begin{figure}[htpb]
			\centering
			\includegraphics[width=0.5\textwidth]{log_likelihood_ratio_kappa_min_dR.png}
			\caption{The profile likelihood ratio scans for $\kappa_\lambda$. The pairing method is $\text{min-}\Delta R$ method. The orange line indicates the $2\sigma$ exclusion boundary.}
			\label{fig:log_likelihood_ratio_kappa_min_dR}
		\end{figure}
		\begin{figure}[htpb]
			\centering
			\includegraphics[width=0.5\textwidth]{log_likelihood_ratio_kappa_SPANET.png}
			\caption{The profile likelihood ratio scans for $\kappa_\lambda$. The pairing method is SPA-NET method. The orange line indicates the $2\sigma$ exclusion boundary.}
