...
\label{sec:dnn_event_classifier}
	...
	\label{sub:train_on_different_kappa}
		The DNNs are trained on different $\kappa_\lambda$ samples, then used to select the samples with $\kappa_\lambda = -5, 11$. The results are shown in Table \ref{tab:DNN_selection_results_different_kappa}. For the DNNs train on different $\kappa_\lambda$ samples show better results on $S / \sqrt{B}$. But there is no such difference between the $\text{min-}\Delta R$ and SPA-NET pairing methods.
		\begin{table}[htpb]
			\centering
			\caption{The DNN selection results with different training and testing samples.}
			\label{tab:DNN_selection_results_different_kappa}
			\resizebox{\textwidth}{!}{
			\begin{tabular}{c|cc|cc|c|ccc}
										   &       &      & \multicolumn{2}{c}{Cross section (fb)} &         & $\mathcal{L} = 139 \text{ fb}^{-1}$ & $\mathcal{L} = 300 \text{ fb}^{-1}$ & $\mathcal{L} = 3000 \text{ fb}^{-1}$ \\
	Pairing method                                & Train & Test & Signal            & Bkg.               & $S / B$ & $S/\sqrt{B}$                        & $S/\sqrt{B}$                        & $S/\sqrt{B}$                         \\ \hline
			\multirow{4}{*}{$\text{min-}\Delta R$} & $1$  & $-5$ & 1.791 & 90.8     & 0.0197 & 2.2159 & 3.2554 & 10.2946 \\
												   & $-5$ & $-5$ & 1.233 & 41.2     & 0.0300 & 2.2657 & 3.3286 & 10.5260 \\
												   & $1$  & $11$ & 1.272 & 90.8     & 0.0140 & 1.5730 & 2.3108 & 7.3075  \\
												   & $11$ & $11$ & 1.061 & 30.8     & 0.0344 & 2.2539 & 3.3113 & 10.4711 \\ \hline
			\multirow{4}{*}{resonant SPA-NET}      & $1$  & $-5$ & 1.196 & 49.0     & 0.0244 & 2.0131 & 2.9575 & 9.3525  \\ 
												   & $-5$ & $-5$ & 1.268 & 36.8     & 0.0345 & 2.4662 & 3.6231 & 11.4573 \\
												   & $1$  & $11$ & 0.778 & 49.0     & 0.0159 & 1.3107 & 1.9255 & 6.0891  \\
												   & $11$ & $11$ & 0.711 & 22.6     & 0.0314 & 1.7611 & 2.5872 & 8.1814  \\ \hline
			\multirow{4}{*}{$\kappa 5$ SPA-NET}    & $1$  & $-5$ & 1.949 & 1.13e+02 & 0.0173 & 2.1630 & 3.1777 & 10.0487 \\
												   & $-5$ & $-5$ & 0.640 & 14.5     & 0.0442 & 1.9832 & 2.9135 & 9.2134  \\
												   & $1$  & $11$ & 1.417 & 1.13e+02 & 0.0126 & 1.5727 & 2.3105 & 7.3064  \\
												   & $11$ & $11$ & 1.073 & 31.7     & 0.0338 & 2.2463 & 3.3000 & 10.4356
			\end{tabular}
			}
		\end{table}
...
\section{DNN classifier with mixing \texorpdfstring{$\kappa_\lambda$}{kappa} samples}% (fold)
\label{sec:dnn_classifier_with_mixing_kappa_samples}
	...
	\label{sub:selection_with_dnn}
		...
		\begin{table}[htpb]
			...
			\begin{tabular}{l|cc|c|ccc}
								  & \multicolumn{2}{c|}{Cross section (fb)} &         & $\mathcal{L} = 139 \text{ fb}^{-1}$ & $\mathcal{L} = 300 \text{ fb}^{-1}$ & $\mathcal{L} = 3000 \text{ fb}^{-1}$ \\
			Pairing               & Signal           & Background           & $S / B$ & $S/\sqrt{B}$                        & $S/\sqrt{B}$                        & $S/\sqrt{B}$                         \\ \hline
			$\text{min-}\Delta R$ & 0.218 & 78.9 & 0.00276 & 0.2893 & 0.4250 & 1.3439 \\
			resonant SPA-NET      & 0.227 & 99.6 & 0.00227 & 0.2675 & 0.3930 & 1.2429 \\
			$\kappa 5$ SPA-NET    & 0.223 & 89.3 & 0.00249 & 0.2779 & 0.4082 & 1.2908
			\end{tabular}
			}
		\end{table}
	...
	\label{sub:kappa_limits}
		...
		\end{table}
	% subsection kappa_limits (end)
	\subsection{Test on different \texorpdfstring{$\kappa_\lambda$}{kappa} samples}% (fold)
	\label{sub:test_on_different_kappa_samples}
		The mixing DNN is used to select the samples with $\kappa_\lambda = -5, 11$. The results are shown in Table \ref{tab:DNN_selection_results_different_kappa2}. For the DNNs train on different $\kappa_\lambda$ samples show better results on $S / \sqrt{B}$. But there is no such difference between the $\text{min-}\Delta R$ and SPA-NET pairing methods.
		\begin{table}[htpb]
			\centering
			\caption{The DNN selection results with different training and testing samples.}
			\label{tab:DNN_selection_results_different_kappa2}
			\resizebox{\textwidth}{!}{
			\begin{tabular}{c|cc|cc|c|ccc}
										   &       &      & \multicolumn{2}{c}{Cross section (fb)} &         & $\mathcal{L} = 139 \text{ fb}^{-1}$ & $\mathcal{L} = 300 \text{ fb}^{-1}$ & $\mathcal{L} = 3000 \text{ fb}^{-1}$ \\
	Pairing method                                & Train & Test & Signal            & Bkg.               & $S / B$ & $S/\sqrt{B}$                        & $S/\sqrt{B}$                        & $S/\sqrt{B}$                         \\ \hline
			\multirow{4}{*}{$\text{min-}\Delta R$}& $1$ & $-5$ & 1.791 & 90.8 & 0.0197  & 2.2159 & 3.2554 & 10.2946 \\
												& mix & $-5$ & 2.030 & 78.9 & 0.0257  & 2.6949 & 3.9591 & 12.5198 \\
												& $1$ & $11$ & 1.272 & 90.8 & 0.0140  & 1.5730 & 2.3108 & 7.3075  \\
												& mix & $11$ & 1.285 & 78.9 & 0.0163  & 1.7062 & 2.5066 & 7.9266  \\ \hline
			\multirow{4}{*}{resonant SPA-NET}   & $1$ & $-5$ & 1.038 & 40.9 & 0.0254  & 1.9148 & 2.8130 & 8.8956  \\
												& mix & $-5$ & 1.832 & 99.6 & 0.0184  & 2.1639 & 3.1790 & 10.0529 \\
												& $1$ & $11$ & 0.778 & 49.0 & 0.0159  & 1.3107 & 1.9255 & 6.0891  \\
												& mix & $11$ & 1.362 & 99.6 & 0.0137  & 1.6082 & 2.3627 & 7.4714  \\ \hline
			\multirow{4}{*}{$\kappa 5$ SPA-NET} & $1$ & $-5$ & 1.611 & 81.1 & 0.0199  & 2.1096 & 3.0993 & 9.8008  \\
												& mix & $-5$ & 1.887 & 89.3 & 0.0211  & 2.3543 & 3.4588 & 10.9376 \\
												& $1$ & $11$ & 1.130 & 81.1 & 0.0139  & 1.4798 & 2.1740 & 6.8747  \\
												& mix & $11$ & 0.852 & 89.3 & 0.00954 & 1.0630 & 1.5617 & 4.9385 
			\end{tabular}
			}
		\end{table}
	...
	% subsection test_on_different_kappa_samples (end)	
% section dnn_classifier_with_mixing_kappa_samples (end)		
\end{document} 
