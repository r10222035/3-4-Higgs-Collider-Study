...
\label{sec:di_higgs_signal_background_analysis}
	...
	\label{sub:atlas}
		...
		\begin{figure}[htpb]
			...
			\caption{The selection acceptance times efficiency at each stage for scalar.}
		...
		\begin{table}[htpb]
			...
			\begin{tabular}{l|cc|cc}
				...
				Delta R       & 0.0900          & 0.9003           & 0.0990            & 0.7139             \\
				Higgs PT      & 0.0683          & 0.7584           & 0.0768            & 0.7759             \\
				Higgs Eta     & 0.0647          & 0.9469           & 0.0719            & 0.9361             \\
				Signal region & 0.0390          & 0.6028           & 0.0431            & 0.5990             \\
				Top veto      & 0.0375          & 0.9612           & 0.0427            & 0.9909            
			\end{tabular}
		\end{table}
		...
		\label{subs:delta_r_cut}
			Change the b-tagging efficiency in Delphes such that the ``Four tag'' efficiency is same as the ATLAS results. The b-tagging efficiency after the change is
		...
		The four b-jets with the highest $p_\text{T}$ are paired to construct two Higgs boson candidates in my samples. When I checked the truth matching, I found the four b-jets may not all decay from Higgs. The event with a b-jet does not decay from Higgs is hard to pass the ``Delta R cut''. The number is in Table \ref{tab:signal_selection_compared_whit_truth_matching_1000GeV}. 
			\begin{table}[htpb]
				\centering
				\caption{The events that cannot define the correct jet assignments are dropped in this table. Use the correct jet assignments to determine if four b-jets are all from Higgs. The total number of events is 100k. The resonant mass is $\text{1000 GeV}$.}
				\label{tab:signal_selection_compared_whit_truth_matching_1000GeV}
				\begin{tabular}{l|ccc}
							 & Total & All from Higgs & Not all from Higgs \\ \hline
					Four tag & 11884 & 9348    & 2536      \\
					Delta R  & 9818  & 8961    & 857        
				\end{tabular}
			\end{table}
