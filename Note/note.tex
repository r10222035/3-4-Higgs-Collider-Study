...
\label{sec:construction_of_bb_pairs}
	...
	Method 2 is better to identify the true Higgs pair.
...
\label{sec:absolute_cross_section}
	...
	I have generated background events with different cuts in MadGraph, the detailed information of cuts is in Table \ref{tab:cut_of_different_run}. I calculate their absolute cross section and check if it is same or not. The result is in Table \ref{tab:absolute_cross_section}.
...
\label{sec:spanet}
	...
	\label{sub:prepare_training_data}
		\begin{enumerate}
			...
			Example: for diHiggs event (h > b b~)
			\begin{verbatim}
				[SOURCE]
				mass = log_normalize
				pt = log_normalize
	...
	\subsection{Training result}% (fold)
	\label{sub:training_result}
		Take diHiggs event as example:
		
		Event purity:
		\begin{equation}
			\epsilon^{\text{event}} \equiv \frac{\text{both Higgs have all jets correctly assigned}}{\text{events with two identifiable Higgs}} 
		\end{equation}

		h purity:
		\begin{equation}
			\epsilon^{\text{h}} \equiv \frac{\text{Higgs have all jets correctly assigned}}{\text{number of identifiable Higgs}} 
		\end{equation}

		For 100k diHiggs event, $\epsilon^{\text{event}} = 0.851, \epsilon^{\text{h}} = 0.922$
	% subsection training_result (end)	
% section spanet (end)	
...
\label{sec:comparision_for_pp_to_hhh_and_gg_to_hhh}
	...
	These events are generated in $\text{14 TeV}$. The cross section for pp->hhh and gg->hhh are $\text{11.22 fb}$ and $\text{11.08 fb}$, respectively. They only differ by 1\%.
...
\label{sec:comparision_for_gg_hhh_and_resonant_channel}
	...
	To compare gg->3h and resonant channel, I plot the $p_\text{T}$, $\eta$ and total invariant mass of 6 b-jets for both.

	The number of events of 6 b-jets for gg->3h is 21,814 and for resonant channel is 21,475. The numbers are very close. I scaled the number of events for gg->3h to be the same as resonant channel.
