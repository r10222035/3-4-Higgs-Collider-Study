...
\label{sec:tri_higgs_signal_background_analysis}
	...
	\label{sub:tri_higgs_selection_step}
		...
		\begin{itemize}
			...
			\item Pick the pairing which minimize the mass difference of 3 Higgs candidates, i.e., minimize
			\[
				\chi^2 = [M(\text{b}_1\text{b}_2) - M(\text{b}_3 \text{b}_4)]^2 + [M(\text{b}_1\text{b}_2) - M(\text{b}_5 \text{b}_6)]^2 +[M(\text{b}_3\text{b}_4) - M(\text{b}_5 \text{b}_6)]^2
			\] 
			\item Signal region:
...
\label{sec:spanet_for_di_higgs_mixing_samples}
	...
	\label{sub:training_sample_preparation}
		...
		The b-tagging part in Delphes card are set such that the efficiency is the same as the MV2c10 b-tagger at 70\% WP. The b-jet eﬀiciency is set to 0.70. The c-jet missing rate is set to 0.083. The light jet missing rate is set to 0.0026.
