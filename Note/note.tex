	...
	\label{sub:atlas}
		...
		\begin{itemize}
			...
			\[
				...
				\right\} \text{ if } m_{\text{4j}} >  \text{1250 GeV}
			\] 
			\item If there are more than 2 pairing satisfy the Delta R requirement. Calculate $D_{HH}$
			\[
			...
			\item Higgs PT: 
			\begin{align*}
				p_{\text{T}}^{\text{lead}} &> 0.5 m_{\text{4j}} - \text{103 GeV} \\
				p_{\text{T}}^{\text{subl}} &> 0.33 m_{\text{4j}} - \text{73 GeV} \\
			\end{align*}
			\item Higgs Eta: 
			\[
		...
		Test the above selection steps on the $\text{13 TeV}$ samples with $m_{H_2} = \text{1000 GeV}$ and compare the results with ATLAS paper (Figure \ref{fig:ATLAS_signal_selection}). The result is in Table .
		\begin{figure}[htpb]
			\centering
			\includegraphics[width=0.8\textwidth]{ATLAS_signal_selection.png}
			\caption{The selection acceptance times efficiency at each stage of the event selection for scalar.}
			\label{fig:ATLAS_signal_selection}
		\end{figure}
		\begin{table}[htpb]
			\centering
			\caption{The selection efficiency at each stage. The resonant mass is $\text{1000 GeV}$. My sample 2 is normalized the Delta R efficiency to 0.09.}
			\label{tab:label}
			\begin{tabular}{l|ccc}
							  & ATLAS  & My sample 1 & My sample 2 \\ \hline
				Four tag      & 0.1000 & 0.1386      & 0.1261      \\
				Delta R       & 0.0900 & 0.0990      & 0.0900      \\
				Higgs PT      & 0.0683 & 0.0768      & 0.0698      \\
				Higgs Eta     & 0.0647 & 0.0719      & 0.0654      \\
				Signal region & 0.0390 & 0.0431      & 0.0392      \\
				Top veto      & 0.0375 & 0.0427      & 0.0388     
			\end{tabular}
		\end{table}
