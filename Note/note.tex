...
\label{sec:branching_ratios_and_decay_widths}
	...
	For color channels, they are still not so close, because in MadGraph it only calculate to LO, but the numbers in Mathematica notebook are calculated to NLO.
...
\section{SPANet for diHiggs event}% (fold)
\label{sec:spanet_for_dihiggs_event}
	\verb+.ini+ file for diHiggs event (h > b $\overline{\text{b}}$)
	\begin{verbatim}
		[SOURCE]
		mass = log_normalize
		pt = log_normalize
	...
	\subsection{100k diHiggs}% (fold)
	\label{sub:100k_dihiggs}
		For 100k diHiggs event:
		\begin{itemize}
			\item Training sample:
			\begin{itemize}
		...
		\end{figure}
		Figure \ref{fig:100k_diHiggs_result} is the training results for 100k diHiggs events. For 2h events, $\epsilon^{\text{event}} = 0.871, \epsilon^{\text{h}} = 0.933$.
	
	% subsection 100k_dihiggs (end)	

	\subsection{1M diHiggs}% (fold)
	\label{sub:1m_dihiggs}
		For 1M diHiggs event:
		\begin{itemize}
			\item Training sample:
			\begin{itemize}
		...
		\begin{figure}[htpb]
			...
			\caption{The training result for 1M diHiggs events.}
			\label{fig:1M_diHiggs_result}
		\end{figure}

		Figure \ref{fig:1M_diHiggs_result} is the training results for 1M diHiggs events. For 2h events, $\epsilon^{\text{event}} = 0.914, \epsilon^{\text{h}} = 0.955$.
	% subsection 1m_dihiggs (end)
