...
\label{sec:spanet}
	...
	\label{sub:training_result}
		...
		Figure \ref{fig:100k_diHiggs_result} is the training results for 100k diHiggs events. There are three types of events, 0h, 1h, 2h. The number means how many identifiable Higgs in a event.
		\begin{figure}[htpb]
			\centering
			\includegraphics[width=0.8\textwidth]{100k_diHigss.png}
			\caption{The training result for 100k diHiggs events.}
		...
		For 2h events, $\epsilon^{\text{event}} = 0.871, \epsilon^{\text{h}} = 0.933$.
...
\label{sec:comparision_for_gg_hhh_and_resonant_channel}
	...
	The number of events of 6 b-jets for gg->3h is 21,814 and for the resonant channel is 21,475. The numbers are very close. I scaled the number of events for gg->3h to be the same as the resonant channel.
...
\label{sec:branching_ratios_and_decay_widths}
	...
	\begin{table}[htpb]
		...
		\begin{tabular}{lll}
			...
			$\text{h}_2\to\text{h}_2\text{h}_2$     & 0.0       & 0.0            \\
			$\text{h}_2\to\text{h}_1\text{h}_2$     & 0.0       & 0.0            \\
			$\text{h}_2\to\chi\chi$       & 0.0       & 0.0        
		\end{tabular}		
		\begin{tabular}{lll}
								  & BR        & Width (GeV) \\
			...
			$\text{h}_3\to\text{h}_2\text{h}_2$ & 0.0       & 0.0         \\
			$\text{h}_3\to\chi\chi$   & 0.0       & 0.0        
		\end{tabular}
	\end{table}
